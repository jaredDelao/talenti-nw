...
export class EstudiosLogisticaComponent implements OnInit, OnDestroy {
  ...
  detalles(data) {
    if (this.banderaSupervisor) {
      this.router.navigate([
        "/logistica/detalle-estudio-supervisor/",
        data.iIdSolicitud,
      ]);
    } else {
      this.router.navigate([
        "/logistica/detalle-estudio-logistica/",
        data.iIdSolicitud,
      ]);
    }
  }
  ...
  exportExcel() {
    ...
    const exportExc = data.reduce((acc, v) => {
      let arr = [
        ...
        this.verificarEstatusAplicacion(
          v.iEstatusGeneral,
          v.bEstatusAsignacion
        ),
      ];
      acc.push(arr);
      return acc;
