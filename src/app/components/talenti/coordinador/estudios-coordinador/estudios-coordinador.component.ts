...
export class EstudiosCoordinadorComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public logisticaService: LogisticaService,
  ...
  detalles(data) {
    this.router.navigate([
      "/coordinador/detalle-estudio-coordinador/",
      data.iIdSolicitud,
    ]);
  }
  ...
  empleadoAsignado(idempleado) {
    const empleado = this.catEmpleados.find(
      (value) => value.iIdEmpleado == idempleado
    );
    if (empleado) {
  ...
  verificarEstatusSolicitud(element) {
    ...
    if (iEstatusComplemento > "0" && iEstatusComplemento != "3")
      complementoPend = true;
    if (iPublicarPreliminar > "0" && iPublicarPreliminar != "3")
      preliminarPend = true;

    if (
      bPublicarDictamen == "2" ||
      iPublicarPreliminar == "2" ||
      iEstatusComplemento == "4"
    )
      return "Revisar";

    if (bPublicarDictamen == "3" && !complementoPend && !preliminarPend)
      return "Validado";
